...
const postCommentToJira = async (issueKey, commentText, issueSelfUrl) => {
  ...
  try {
    // Prefer JIRA_DOMAIN env var (clean domain, no board paths)
    // Fall back to parsing the issueSelfUrl if domain not set
    let origin = '';
    const rawDomain = process.env.JIRA_DOMAIN || '';
    if (rawDomain) {
      try {
        origin = new URL(rawDomain.startsWith('http') ? rawDomain : 'https://' + rawDomain).origin;
      } catch (_) {
        origin = 'https://' + rawDomain.replace(/^https?:\/\//i, '').split('/')[0];
      }
    } else if (issueSelfUrl) {
      try { origin = new URL(issueSelfUrl).origin; } catch (_) {}
    }

    if (!origin) {
      console.warn(`⚠️ Cannot determine Jira origin for comment — skipping.`);
      return;
    }

    const commentUrl = `${origin}/rest/api/2/issue/${issueKey}/comment`;
